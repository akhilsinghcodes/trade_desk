...
def render_analyze_page(st_obj, ticker, period, show_bb, show_sma, run_backtest, fast_win, slow_win):
    ...
    ml_verdict = analysis.get("ml_verdict")
...
""", unsafe_allow_html=True)
    ...
    # ── SECTION 1.6: ML MODEL PREDICTION + THIS TICKER'S OWN TRACK RECORD ──────
    # Separate model from the composite above — an XGBoost regressor trained on
    # 149 stocks, evaluated PER-TICKER (walk-forward, embargo-correct), not as a
    # cross-sectional ranker. Runs live in-process for ANY ticker; only the 149
    # trade_ml has backtested get a real per-ticker track record, everything
    # else shows the model's honest aggregate stats instead.
    if ml_verdict is not None:
        _ml_ret = ml_verdict["pred_return_5d"]
        _ml_color = "#4ade80" if _ml_ret > 0 else "#f87171" if _ml_ret < 0 else "#94a3b8"
        _ml_target_price = close_price * (1 + _ml_ret)

        # Agreement flag — backtested (151,745 OOS rows, 73 tickers, walk-forward):
        # when this composite banner and the ML model's direction agree, the ML
        # model's Spearman rho is +0.072 (p<0.0001); when they disagree, it's
        # -0.014 (p=0.004) — statistically dead. Disagreement isn't "two votes
        # canceling out," it's the regime where neither signal should be trusted.
        _ml_dir = "up" if _ml_ret > 0 else "down" if _ml_ret < 0 else None
        _banner_dir = "up" if _verdict_word == "BUY" else "down" if _verdict_word == "SELL" else None
        if _banner_dir and _ml_dir:
            if _banner_dir == _ml_dir:
                _agreement_html = (
                    '<div style="margin-top:8px;padding:6px 10px;background:rgba(74,222,128,0.12);'
                    'border-radius:6px;font-size:0.75rem;color:#4ade80">'
                    '✓ Agrees with the composite banner above — backtested high-confidence regime '
                    '(ρ=+0.072 when signals agree vs -0.014 when they disagree)</div>'
                )
            else:
                _agreement_html = (
                    '<div style="margin-top:8px;padding:6px 10px;background:rgba(248,113,113,0.12);'
                    'border-radius:6px;font-size:0.75rem;color:#f87171">'
                    '⚠ Disagrees with the composite banner above — backtested low-confidence regime, '
                    'neither signal has shown real edge when they diverge (ρ≈-0.014, not significant '
                    'in the useful direction)</div>'
                )
        else:
            _agreement_html = ""

        if ml_verdict["has_track_record"]:
            _panel_label = "ML Model Prediction (per-ticker validated)"
            if ml_verdict["track_significant"]:
                _track_color = "#4ade80" if ml_verdict["track_direction"] == "positive" else "#f87171"
                _track_text = (
                    f'<span style="color:{_track_color};font-weight:600">'
                    f'this ticker: ρ={ml_verdict["track_rho"]:+.3f} (p={ml_verdict["track_pval"]:.3f}), '
                    f'significant over {ml_verdict["track_n_folds"]} folds</span>'
                )
            else:
                _track_text = (
                    f'<span style="color:#e2c882">this ticker: ρ={ml_verdict["track_rho"]:+.3f} '
                    f'(p={ml_verdict["track_pval"]:.3f}) — not statistically significant, treat as noise</span>'
                )
            _footer = (
                "Track record is this ticker's OWN historical correlation between this model's "
                "predictions and actual returns — not the model's average across all stocks. A "
                "model can be real on average and still be noise for one particular stock."
            )
        else:
            _panel_label = "ML Model Prediction (live)"
            _n = ml_verdict.get("agg_n_tickers_tested")
            if _n:
                _track_text = (
                    f'<span style="color:#e2c882">{ticker} hasn\'t been individually backtested — '
                    f'model\'s aggregate record across its {_n}-ticker tested universe: '
                    f'{ml_verdict["agg_n_positive"]}/{_n} positive, '
                    f'{ml_verdict["agg_n_significant"]}/{_n} significant, '
                    f'median ρ={ml_verdict["agg_median_rho"]:+.3f}</span>'
                )
            else:
                _track_text = '<span style="color:#e2c882">No track record data available.</span>'
            _footer = (
                "Same trained model, run live for this ticker instead of pre-backtested. The "
                "prediction is real; what's untested is whether it works for THIS ticker "
                "specifically — the numbers above are the model's typical performance elsewhere."
            )
        st_obj.markdown(f"""
<div style="border:1px solid {_ml_color}40;background:{_ml_color}0d;border-radius:12px;padding:16px 22px;margin-bottom:12px">
  <div style="font-size:0.75rem;opacity:0.6;text-transform:uppercase;letter-spacing:0.05em">{_panel_label}</div>
  <div style="display:flex;align-items:baseline;gap:12px;margin-top:6px;flex-wrap:wrap">
    <span style="font-size:1.5rem;font-weight:700;color:{_ml_color};font-family:'JetBrains Mono',monospace">~${_ml_target_price:.2f} ({_ml_ret:+.2%})</span>
    <span style="opacity:0.6;font-size:0.85rem">predicted 5-day forward price, as of {ml_verdict["as_of"]}</span>
  </div>
  <div style="margin-top:8px;font-size:0.8rem">{_track_text}</div>
  {_agreement_html}
  <div style="margin-top:6px;font-size:0.7rem;opacity:0.4">{_footer}</div>
</div>
""", unsafe_allow_html=True)
    else:
        st_obj.caption(f"ℹ️ Couldn't get an ML prediction for {ticker} — likely insufficient price history.")
