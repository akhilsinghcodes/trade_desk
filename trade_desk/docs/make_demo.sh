...
async def set_ticker(page, ticker, wait_ms=8000):
    ...
    await ticker_box.wait_for(state="visible", timeout=15000)
    # Streamlit disables the input while a previous rerun is still in flight —
    # wait it out instead of racing it.
    for _ in range(60):
        if await ticker_box.is_enabled():
            break
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        if await btn.count() > 0:
            ...
            # The dismissed disclaimer's stDialog element stays in the DOM
            # (hidden) and can still intercept clicks at some scroll positions.
            await page.evaluate("document.querySelectorAll('[data-testid=\"stDialog\"]').forEach(e => e.remove())")
        ...
        # ML Model Prediction — XGBoost regressor, published from trade_ml,
        # run live in-process here. Shows per-ticker validated track record
        # (or the model's aggregate stats for tickers outside the 149-ticker
        # backtested universe) plus the agreement flag vs the composite verdict.
        print("→ ML Model Prediction panel")
        await page.evaluate("window.scrollTo(0, 380)")
        await shot(page, "ml_model_prediction", 3)
        ...
        if await corr_expander.count() > 0:
            await corr_expander.click(force=True)
            await page.wait_for_timeout(800)
        ...
        print("→ Model Ledger")
        await nav_to(page, "🧾 Model Ledger")
        await page.wait_for_timeout(2000)
        await shot(page, "model_ledger", 3)
